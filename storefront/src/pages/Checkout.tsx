...
import { errorBannerClass, formatInr, primaryButtonClass } from '../lib/ui'
...
export default function Checkout() {
  ...
  const { authed, plan, school, order, setOrder, setLicense } = usePurchase()
  ...
  useEffect(() => {
    if (!authed || !plan || !school || order) return
    api.createOrder(plan.id, school.tenantId)
      .then(setOrder)
  ...
  if (!plan || !school) return <Navigate to="/" replace />

  const pay = async () => {
    if (!order) return
    setBusy(true); setError('')
    try {
      const license = await api.verify(order.order_id)
      setLicense(license)
      nav('/success')
    } catch (e) {
  ...
  const retryAfterFailure = async () => {
    setShowModal(false); setFailed(true); setOrder(null)
    try {
      setOrder(await api.createOrder(plan.id, school.tenantId))
    } catch (e) {
      setError(e instanceof ApiError ? e.detail : 'Could not create order')
    }
  }
        ...
        <div className="w-full max-w-[420px] rounded-[20px] border border-mist/[0.08] bg-ink-card p-8">
          ...
          <div className="mt-6 rounded-[12px] border border-lime/20 bg-moss/15 p-4">
            ...
            <div className="mt-1.5 flex items-baseline justify-between">
              ...
              <span className="text-[15px] font-semibold">{formatInr(price)}/yr</span>
            </div>
            <p className="mt-1 text-[12px] text-sage">{school.name} • {plan.included_seats} devices</p>
          </div>

          <div className="mt-5 space-y-2.5 text-[13px]">
            <div className="flex justify-between text-sage">
              <span>Subscription (annual)</span><span>{formatInr(base)}</span>
            </div>
            <div className="flex justify-between text-sage">
              <span>GST (18% inclusive)</span><span>{formatInr(gst)}</span>
            </div>
            <div className="h-[1px] bg-mist/[0.08] my-2" />
            <div className="flex justify-between text-[15px] font-semibold">
              <span>Total due today</span><span>{formatInr(price)}</span>
            </div>
          </div>
          ...
          <p className="mt-2 text-center">
            <Link to="/onboarding" className="text-[12px] text-sage-dim hover:text-mist transition">← Edit school details</Link>
          </p>
        </div>
      </main>

      {showModal && order && (
        <RazorpayModal order={order} schoolName={school.name}
